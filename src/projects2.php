...
&#160;&#160;
            ...
            <input name="submitme" type="submit" value="GO" class="btn btn-primary">
</div>
</form>
<br />
...
if($city2==('all')){
	if($category2=='all'){
		$query = "SELECT pid,sid,startupname,proname,stipened,city, applyby FROM addpro WHERE (applyby >= CURDATE()) ORDER BY YEAR(applyby) ASC, MONTH(applyby) ASC, DAY(applyby) ASC ";
		$result = mysqli_query($conn,$query);
	}
	else{
		$query = "SELECT pid,sid,startupname,proname,stipened,city, applyby FROM addpro WHERE (category = ?) AND(applyby >= CURDATE()) ORDER BY YEAR(applyby) ASC, MONTH(applyby) ASC, DAY(applyby) ASC ";
	...
	if($category2=='all'){
		$query = "SELECT pid,sid,startupname,proname,stipened,city, applyby FROM addpro WHERE (city = ?) AND  (applyby >= CURDATE()) ORDER BY YEAR(applyby) ASC, MONTH(applyby) ASC, DAY(applyby) ASC ";
			$stmt=$conn->prepare($query);
			$stmt->bind_param('s',$a);
			$a=$city2;
			$stmt->execute();
	...
	$query = "SELECT pid,sid,startupname,proname,stipened,city, applyby FROM addpro WHERE (applyby >= CURDATE()) AND (city = ? AND category = ?) ORDER BY YEAR(applyby) ASC, MONTH(applyby) ASC, DAY(applyby) ASC ";
			$stmt=$conn->prepare($query);
...
<table class='table table-striped table-hover ' text-align='center'>
  <thead>
    <tr><td colspan='3'><h4>Startup: ". $arry['startupname']."</h4></td></tr>
  </thead>
  <tbody>
    <tr>
    </tr>
   <tr><td colspan='3'><h4>Project: " . $arry['proname']."</h4></td></tr>
    <tr >
    </tr>
    <tr><td>Stipened: " . $arry['stipened']."</td>
<td>Location: " . $arry['city']."</td>
<td text-align='center'>Apply by: " . date_format($a,'d/m/Y')."</td></tr>
    <tr >
    </tr>
	 <tr><td colspan='3'><a href='prodet.php?id=".$arry['pid']."' id= 'vp' class='btn btn-default btn-lg btn-block'>View Project</a></td></tr>
  </tbody>
</table> 
</div>";
}
